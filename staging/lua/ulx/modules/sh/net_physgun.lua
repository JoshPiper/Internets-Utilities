...
--[[ Code Shamelessly borrowed from ULX / Bit of my own.]]--
...
function ulx.unfreezeDead(calling_ply)
	local effected = {}
	local i = 0
	for k, v in pairs(players.GetAll()) do
		if v.freezingPly then
			table.insert(effected, v)
			v:freeze(false)
	...
	ULib.tsay(calling_ply, "Unfroze "..i.." players.")
	if not i == 0 then
		for k, v in pairs(effected) do
			ULib.console(calling_ply, "Unfroze "..v:Name())
		end
	end
end
local unfreezeall = ulx.command("Utility", "ulx ulxunfreezeall", ulx.unfreezeDead, "!unfreezeall")
unfreezeall:defaultAccess(ULib.ACCESS_ADMIN)
unfreezeall:help("Unfreezes all players who have been physgun frozen. (Use if someone is stuck dead and frozen)")

function ulx.onPhysgunReload(wpn, ply)
	local target = ply:GetEyeTrace().Entity
	if target and target:IsValid() and target:IsPlayer() then
		if target.frozen and target.freezingPly and target.freezingPly == ply then
			target:UnLock()
			target:SetMoveType(MOVETYPE_WALK)
			target.freezingPly = nil
			target.frozen = false
			ulx.clearExclusive(ent)
			return true
		elseif ply:query("ulx unfreezeany") then
			target:UnLock()
			target:SetMoveType(MOVETYPE_WALK)
